...
type Section = "dashboard" | "crops" | "scan" | "cases" | "profile" | "farmers" | "analytics" | "scans" | "experts" | "stores" | "risks";
...
function LocalAuthPanel({ navigate, language }: { navigate: (path: string) => void; language: LanguageCode }) {
  ...
  const detectSignupLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      setGpsState("saved");
      toast.success("GPS location captured. Address fields will fill automatically when the location resolves.");
    }, () => {
      setGpsState("denied");
...
export default function Home() {
  ...
  const primaryMobileItems = mobileNavItems.filter((item) => role === "farmer" ? ["dashboard", "crops", "scans", "cases"].includes(item.id) : ["dashboard", "scans", "cases", "farmers"].includes(item.id));
